package com.example.managework.mvvmtodolist.viewmodel

import android.app.Application
import androidx.lifecycle.AndroidViewModel
import androidx.lifecycle.LiveData
import androidx.lifecycle.viewModelScope
import com.example.managework.mvvmtodolist.database.TaskDatabase
import com.example.managework.mvvmtodolist.database.TaskEntry
import com.example.managework.mvvmtodolist.repository.TaskRepository
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class TaskViewModel(application: Application) : AndroidViewModel(application) {

    private val taskDao = TaskDatabase.getDatebase(application).taskDao()
    ...
    fun delete(taskEntry: TaskEntry){
        viewModelScope.launch(Dispatchers.IO){
            repository.deletaItem(taskEntry)
        }
    }
